...
public class RepeatedAStar{
	...
	public static ArrayList<Node> adaptivePlanning(Node current, boolean isForward, int[][] blocked){
		if(!isForward && current.getX() == 0 && current.getY() == 0){
			current.setX(100);	
			current.setY(100);
		}
		...
		if(isForward){
			while(current.getX() != 100 || current.getY() != 100){
				...
				newNode.setY(current.getY());
				newNode.setG(current.getG());
				newNode.setH(getHAdaptive(current,dest));
				closedList.add(newNode);
				inClosedList = false;
				isBlocked = false;
				temp = root;
	...
	public static ArrayList<Node> executePath(boolean isForward, Grid grid, boolean isAdaptive){
		int count = 0;
		...
		if(!isAdaptive){
			plannedRoute = planning(current, isForward, blocked);
		}
		else {
			plannedRoute = adaptivePlanning(current,isForward,blocked);
		}
		...
		while(true){
			...
			if(!isAdaptive){
				...
				plannedRoute = adaptivePlanning(newNode,isForward,blocked);
			}
			ranIntoBlock = false;
			if(plannedRoute == null){
				return null;
	...
	public static int getHAdaptive(Node start, Node dest){
		int ha = Math.abs(dest.getG() - start.getG());
		return ha;
	}
}